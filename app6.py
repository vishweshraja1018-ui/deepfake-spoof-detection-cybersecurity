...
def gradcam_focus_score(cam_pil: Image.Image):
    """
    Heuristic: how concentrated is attention?
    Returns dict: focus (0..1), peaks (fraction of top-hot pixels).
    """
    if cam_pil is None:
        return None

    # overlay image contains heat colors; take "redness" as proxy for heat
    ...
    # Focus = share of heat contained in top 20% hottest pixels
    flat = heat.reshape(-1)
    if flat.size < 10:
        return None
    ...
    focus = float(np.clip(top_sum / total, 0, 1))

    # Peaks = fraction above 90th percentile (very hot)
...
def build_explanation(label: str, conf: float, ai_p: float, real_p: float, cam_metrics):
    """
    Creates human-readable 'Why AI/Why Real' bullets using simple cues:
    - confidence and probability gap
    - gradcam focus/spread (if available)
    """
    ...
    # Confidence-based bullets
    if conf_level == "high":
        bullets.append("The model shows high confidence in this decision (strong probability separation).")
    elif conf_level == "medium":
        bullets.append("The model decision is moderately confident; the probabilities are separated but not extreme.")
    else:
        bullets.append("The model confidence is low; the probabilities are close, so manual review is recommended.")

    bullets.append(f"Probability gap between classes is {gap:.2%}, which reflects how strongly the model favors one class.")

    # Grad-CAM-based bullets
    if cam_metrics is None:
        bullets.append("Grad-CAM explanation is not available for this run, so attention-based reasoning is limited.")
    else:
        focus = cam_metrics["focus"]
        peaks = cam_metrics["peaks"]

        if focus >= 0.62:
            bullets.append("Model attention is concentrated in specific regions, suggesting it relied on localized visual cues.")
        elif focus >= 0.48:
            bullets.append("Model attention is moderately focused; it used a mix of localized and global cues.")
        else:
            bullets.append("Model attention is spread across the image, which often happens when cues are weak or distributed.")

        if peaks >= 0.12:
            bullets.append("There are strong attention peaks, indicating the model found distinct high-signal areas.")
        else:
            bullets.append("Attention peaks are mild; the model did not find extremely strong localized signals.")

    # Verdict-specific explanation
    if label == "ai":
        bullets.append("AI-generated images often contain subtle texture inconsistencies or synthetic patterns that CNNs learn to detect.")
        action = "If this content is high-stakes, verify using the original source file and metadata (EXIF) and run on multiple images."
    else:
        bullets.append("Real photographs generally show consistent sensor noise and natural detail transitions that models can learn as authentic cues.")
        action = "If you expected AI here, re-check dataset bias and consider retraining with more similar AI samples."

    # Action rules
    if conf < DF_THRESHOLD:
        action = "Recommended: treat as uncertain. Try a higher-quality image and verify via source/metadata."
...
# ── RIGHT: Highlight AI/REAL + WHY card ───────────────────────────────────────
with col_right:
    ...
    if not uploaded:
        ...
        if blocked:
            ...
            # WHY AI / WHY REAL card
            bullets, action = build_explanation(
                label=label if not uncertain else ("ai" if ai_p > real_p else "real"),
                conf=conf,
                ai_p=ai_p,
                real_p=real_p,
